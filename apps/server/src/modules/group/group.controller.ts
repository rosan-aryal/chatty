...
export class GroupController {
  ...
  listMyGroups = async (c: Context) => { const user = c.get("user"); return c.json(await this.groupService.listUserGroups(user.id)); };

  joinPublic = async (c: Context) => {
    const user = c.get("user"); const { id } = c.req.param();
  ...
  joinByCode = async (c: Context) => {
    const user = c.get("user"); const { code } = await c.req.json<{ code: string }>();
  ...
  kick = async (c: Context) => {
    const user = c.get("user"); const { id } = c.req.param(); const { userId } = await c.req.json<{ userId: string }>();
